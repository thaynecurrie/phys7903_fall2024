...
import numpy as np
d_tel = 8.2 #telescope diameter (meters)
lam_tel= 1.6 #wavelength (microns) note: we don't call it lambda because that is a separate, canned Python thing
...
and a wavelength of {1:.1f} microns is {2:.4f}".format(d_tel,lam_tel,fwhm))
```
