...
def runme(npca=5):
 ...
 ndim = 10
 mu = np.array([3]+np.random.rand(ndim)) # Mean of 3, randomized +/- 1

#producing the covariance ... set off-diagonal term to be 5
 sigma=np.full((ndim,ndim),5+np.random.rand(ndim*ndim))
...
#similarly sort the eigenvectors
 ...
 ax[0].set_title("Scatter plot of original data")
 ax[1].set_title("Scatter plot of reconstructed data")
 ax[2].set_title("Scatter plot of the original-reconstructed data")
 ax[0].grid('on')
 ax[1].grid('on')
 ax[2].grid('on')
 plt.show()
