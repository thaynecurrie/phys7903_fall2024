...
def prob1():
 ...
 xarray=np.arange(50)
 #xarray=2.1*x**2 +5
#np.arange(50)   #an array of numbers from 0 to 19
 yarray=0.2*xarray**2.+20*np.random.rand(50)
 #y is same as x EXCEPT now we vary the value +/- some random number about x
 
 #scatter plot of xarray and yarray
 plt.scatter(xarray,yarray,marker='v')
 #plt.scatter(xarray,np.arange(50),c='tab:orange')

 def func(x,a,b):
  return a*x**2+b

 param,_=curve_fit(func,xarray,yarray)

 plt.plot(xarray,func(xarray,param[0],param[1]),c='orange',linestyle='-')
 #a,b=np.polyfit(xarray,yarray,2)
 #plt.plot(xarray*a+b)

 np.savetxt('file.txt',np.column_stack((xarray,yarray)), fmt="%.3f" " " "%.2f")
 print(param) 
 #show the plot
 plt.show()
